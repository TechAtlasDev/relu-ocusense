# 🛠️ Backend Técnico - ReLU Bot (Neuroalianza Perú)
...
El backend está construido en **Python 3.12+** utilizando una arquitectura orientada a grafos con **LangGraph** y la API de **Google Gemini** (`gemini-3.6-flash`), integrada asíncronamente con la **Telegram Bot API**.
...
graph TD
    ...
    LangGraph <--> Tools[⚙️ app/tools/: Neuro-Tamizaje / Referencia / Seguimiento / Historia]
```
...
| **`app/services/agent.py`** | Grafo `StateGraph` de LangGraph, memoria persistente por usuario (`MemorySaver`) y `RELU_SYSTEM_PROMPT`. |
